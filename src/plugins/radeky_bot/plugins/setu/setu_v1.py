...
@send_picture.handle()
async def en_handler(bot: Bot, event: GroupMessageEvent, state: T_State):
    await handler(bot, event, state)
    await send_picture.finish()


@send_picture_cn.handle()
async def cn_handler(bot: Bot, event: GroupMessageEvent, state: T_State):
    await handler(bot, event, state)
...
async def handler(bot: Bot, event: GroupMessageEvent, state: T_State):
