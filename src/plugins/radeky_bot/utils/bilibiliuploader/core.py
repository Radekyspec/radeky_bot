...
def upload_chunk(upload_url, server_file_name, local_file_name, chunk_data, chunk_size, chunk_id, chunk_total_num):
    ...
    """
    logger.info("chunk{}/{}".format(chunk_id, chunk_total_num))
    logger.info("filename: {}".format(local_file_name))
    files = {
...
def upload_video_part(access_token, sid, mid, video_part: VideoPart, max_retry=5):
    ...
    logger.debug(file_hash.hexdigest())
...
def upload(access_token,
    ...
    with ThreadPoolExecutor(max_workers=thread_pool_workers) as tpe:
        ...
        for video_part in parts:
            logger.debug("upload {} added in pool".format(video_part.title))
        ...
        for t_obj in as_completed(t_list):
            ...
            logger.info("video part {} finished, status: {}".format(t_obj.video_part.title, t_obj.result()))
            if not status:
    ...
    logger.debug("submit")
    logger.debug(r.status_code)
    logger.info("Submit result: ", r.content.decode())
...
def edit_videos(
    ...
    if not avid and not bvid:
        logger.error("please provide avid or bvid")
        return None, None
    if not avid:
    ...
    if type(insert_index) is int:
        ...
        logger.error("wrong insert index")
        return None, None
